...
#define paternVersion "ccs-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

CcSVersionChecker::CcSVersionChecker(QObject *parent) : QObject(parent)
{
...
bool CcSVersionChecker::newVersionAvailable()
{
...
QList<Version> CcSVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(CCS_RELEASES)));
    QEventLoop event;
...
Version CcSVersionChecker::getMaxReleaseVersion()
{
