...
void initState(){
    ...
    const std::string dirCcS = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<CcSState>(new CcSState(dev::u256(0), CcSState::openDB(dirCcS, hashDB, dev::WithExisting::Trust), dirCcS + "/ccsDB", dev::eth::BaseState::Empty));
...
dev::Address createCcSAddress(dev::h256 hashTx, uint32_t voutNumber){
...
CcSTransaction createCcSTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout){
    CcSTransaction txEth;
    if(recipient == dev::Address()){
        txEth = CcSTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = CcSTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<CcSTransaction> txs){
