...
void checkResult(bool isCreation, std::vector<CcSTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    CcSTxConverter converter(transaction, NULL);
    ExtractCcSTX ccsTx;
    BOOST_CHECK(converter.extractionCcSTransactions(ccsTx));
    std::vector<CcSTransaction> result = ccsTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(ccstxconverter_tests, TestingSetup)
