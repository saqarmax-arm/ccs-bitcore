...
CcSState::CcSState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = CcSState::openDB(_path + "/ccsDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

CcSState::CcSState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute CcSState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, CcSTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createCcSAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            ccs::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> CcSState::vins() const // temp
{
...
void CcSState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* CcSState::vin(dev::Address const& _a) const
{
    return const_cast<CcSState*>(this)->vin(_a);
}

Vin* CcSState::vin(dev::Address const& _addr)
{
...
// void CcSState::commit(CommitBehaviour _commitBehaviour)
// {
...
void CcSState::kill(dev::Address _addr)
{
...
void CcSState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
const dev::Address CcSState::createCcSAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void CcSState::deleteAccounts(std::set<dev::Address>& addrs){
...
void CcSState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void CcSState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
