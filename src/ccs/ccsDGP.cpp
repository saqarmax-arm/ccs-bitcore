...
bool CcSDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule CcSDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t CcSDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t CcSDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t CcSDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t CcSDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool CcSDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void CcSDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void CcSDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void CcSDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void CcSDGP::createParamsInstance(){
...
dev::Address CcSDGP::getAddressForBlock(unsigned int blockHeight){
...
void CcSDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void CcSDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void CcSDGP::parseStorageOneUint64(uint64_t& value){
...
void CcSDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule CcSDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
