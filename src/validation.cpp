...
#if defined(NDEBUG)
# error "CcS cannot be compiled without assertions."
#endif
...
std::unique_ptr<CcSState> globalState;
...
const std::string strMessageMagic = "CcS Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // ccs
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = ccsDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = ccsDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            CcSTxConverter converter(tx, NULL);
            ExtractCcSTX resultConverter;
            if(!converter.extractionCcSTransactions(resultConverter)){
            ...
            std::vector<EthTransactionParams> ccsETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(CcSTransaction ccsTransaction : ccsTransactions){
                sumGas += ccsTransaction.gas() * ccsTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, ccsTransaction.gasPrice());
                } else {
                    txMinGasPrice = ccsTransaction.gasPrice();
                }
                VersionVM v = ccsTransaction.getVersion();
                if(v.format!=0)
                ...
                if(ccsTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(ccsTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(ccsTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)ccsTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(ccsETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > ccsTransactions.size())
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s CCS additional fees, %d delta bytes\n",
...
static DisconnectResult DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // ccs
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // ccs
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    CcSTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<CcSTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path ccsDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(ccsDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(ccsDir.string(), std::ios::in | std::ios::out);
...
bool CcSTxConverter::extractionCcSTransactions(ExtractCcSTX& ccstx){
    std::vector<CcSTransaction> resultTX;
    ...
    ccstx = std::make_pair(resultTX, resultETP);
    return true;
}

bool CcSTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool CcSTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
CcSTransaction CcSTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    CcSTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = CcSTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = CcSTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // ccs
    CcSDGP ccsDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setCcSSchedule(ccsDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = ccsDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = ccsDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = ccsDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // ccs
...
///////////////////////////////////////////////////////////////////////////////////////// ccs
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            CcSTxConverter convert(tx, &view, &block.vtx);

            ExtractCcSTX resultConvertCcSTX;
            if(!convert.extractionCcSTransactions(resultConvertCcSTX)){
            ...
            if(!CheckMinGasPrice(resultConvertCcSTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertCcSTX.first, blockGasLimit);
            ...
            for(CcSTransaction& qtx : resultConvertCcSTX.first){
            ...
            if (fLogEvents)
            {
                for(size_t k = 0; k < resultConvertCcSTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertCcSTX.first[k].from(), resultConvertCcSTX.first[k].to(),
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace)
{
    ...
    LogPrint("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // ccs
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // ccs
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // ccs
            globalState->setRootUTXO(oldHashUTXORoot); // ccs
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        ///////////////////////////////////////////////////////////////////// // ccs
        uint32_t sizeBlockDGP = ccsDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for CcS PoS we need to sync this after every block to ensure txdb is populated for
