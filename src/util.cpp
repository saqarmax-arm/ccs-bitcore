...
const char * const BITCOIN_CONF_FILENAME = "ccs.conf";
const char * const BITCOIN_PID_FILENAME = "ccsd.pid";
...
void OpenDebugLog()
{
    ...
    assert(fileout == NULL);
    assert(fileoutVM == NULL); // ccs
    assert(vMsgsBeforeOpenLog);
    boost::filesystem::path pathDebug = GetDataDir() / "debug.log";
    boost::filesystem::path pathDebugVM = GetDataDir() / "vm.log"; // ccs
    fileout = fopen(pathDebug.string().c_str(), "a");
    fileoutVM = fopen(pathDebugVM.string().c_str(), "a"); // ccs
    if (fileout) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CcS
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CcS
    // Mac: ~/Library/Application Support/CcS
    // Unix: ~/.ccs
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CcS";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CcS";
#else
    // Unix
    return pathRet / ".ccs";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("CcS Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The CcS Core developers";
    }
