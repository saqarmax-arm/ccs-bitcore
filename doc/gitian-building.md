...
*Setup instructions for a Gitian build of CcS Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the CcS
...
- [Building CcS Core](#building-ccs-core)
...
Clone the git repositories for ccs and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/ccsproject/ccs --recursive
...
```bash
sudo cp /home/debian/ccs/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the ccs repository under 'Fetch and create inputs' to install sources which require
...
To build CcS Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the ccs repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ccs/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/ccsproject/ccs
...
For example:
```bash
URL=https://github.com/laanwj/ccs.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit ccs=${COMMIT} --url ccs=${URL} ../ccs/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ccs=${COMMIT} --url ccs=${URL} ../ccs/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ccs=${COMMIT} --url ccs=${URL} ../ccs/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-ccs:
./bin/gbuild --commit ccs=${COMMIT},cpp-eth-ccs=${ETHCOMMIT} --url ccs=${URL},cpp-eth-ccs=${ETHURL} ../ccs/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the ccs git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../ccs/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/ccsproject/ccs-detached-sigs.git

BTCPATH=/some/root/path/ccs
SIGPATH=/some/root/path/ccs-detached-sigs

./bin/gbuild --url ccs=${BTCPATH},signature=${SIGPATH} ../ccs/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ccs-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ccs-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/ccs-osx-build.assert
```
...
[ccsproject/gitian.sigs](https://github.com/ccsproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@ccs.org) and he will commit them.
